...
public class Blame {
    ...
    public static func buffer(reference: OpaquePointer, bufferString: String, buffer_len: Int) -> OpaquePointer? {
    ...
    public static func file(repo: OpaquePointer, pathString: String, options: UnsafeMutablePointer<git_blame_options>) -> OpaquePointer? {
        var out: OpaquePointer? = nil
    ...
    public static func free(blame: OpaquePointer) {
        git_blame_free(blame)
    }
    
    public static func get_hunk_byindex(blame: OpaquePointer, index: UInt32) -> git_blame_hunk? {
        git_blame_get_hunk_byindex(blame, index).pointee
    }
    
    public static func get_hunk_byline(blame: OpaquePointer, lineno: Int) -> git_blame_hunk? {
        git_blame_get_hunk_byline(blame, lineno).pointee
    }
    
    public static func get_hunk_count(blame: OpaquePointer) -> UInt32 {
        git_blame_get_hunk_count(blame)
    }
    
    public static func init_options(version: UInt32) -> git_blame_options {
